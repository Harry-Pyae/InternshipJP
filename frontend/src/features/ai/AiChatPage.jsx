...
export default function AiChatPage({ audience }) {
  ...
  const title = isEmployer ? "Employer assistant" : "Student assistant";
      ...
      <div className="ijp-chat-shell">
        ...
        <div className="ijp-card p-4">
          <EmptyState
            ...
            hint={`You are signed in as ${user.role}. ${title} is only available to ${
              isEmployer ? "employers" : "students"
            }.`}
          />
        </div>
      </div>
  ...
  const starters = isEmployer
    ? employerMode === "company"
      ...
      : [
          "Who is the strongest applicant, and why?",
          "What should I ask each of them in an interview?",
          "Which required skills is this pool missing?",
        ]
    : [
        "What should I learn next to be hired as a backend intern?",
        "What is missing from my profile that employers look for?",
        "Which open internship fits me best right now?",
      ];
    ...
    <div className="ijp-chat-shell">
      <PageHeader
        ...
        subtitle={
          isEmployer
            ? employerMode === "company"
              ? "Review your own listings, pipeline and requirements."
              : "Compare the people who applied, using verified information only."
            : "What to learn next, what is missing from your profile, and which internships fit."
        }
        action={
